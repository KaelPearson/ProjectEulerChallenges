var amountOfDigits = 13;
var number = "";

/* function getHighestAdjacent(){
    var numArr = [];
    var highestNumArr = [];
    var highest = 0;
    for(var i = 0; i < amountOfDigits; i++){
        numArr.push("0");
        highestNumArr.push("0");
    }
    for(var i = amountOfDigits; i < number.length + amountOfDigits; i++){
        for(var k = i; k < (i + amountOfDigits); k++){
            numArr[k] = number[i-amountOfDigits];
        }
        if(getProduct(numArr) > highest){
            highest = getProduct(numArr);
            highestNumArr = numArr;
        }
    }
    return highest;
} */
console.log(getHighestAdjacent());
function getHighestAdjacent(){
    var numArr = ["","","","","","","","","","","","",""];
    var highestNumArr = ["","","","","","","","","","","","",""];
    var highest = 0;

    for(var i = amountOfDigits; i < number.length + amountOfDigits; i++){
        for(var k = 0; k < amountOfDigits; k++){
            numArr[k] = number[i + k - amountOfDigits];
        }
        if(getProduct(numArr) > highest){
            highest = getProduct(numArr);
            for(var k = 0; k < amountOfDigits; k++){
                highestNumArr[k] = numArr[k];
            }
        }
    }
    return highest;
}
function getProduct(arr){
    var total = 1;
    for(var i = 0; i < arr.length; i++){
        total *= parseInt(arr[i]);
    }
    return total;
}